...
template <typename tCoste>
GrafoP<tCoste> Kruskall(const GrafoP<tCoste>& G, const Particion& islas, int Nparticiones)
// Devuelve un árbol generador de coste mínimo
// de un grafo no dirigido ponderado y conexo G.
{
   assert(!G.esDirigido());
   ...
   size_t i = 1;
   while (i <= Nparticiones) {   // Seleccionar n-1 aristas.
      arista a = A.cima(); // arista de menor coste
      A.suprimir();
      vertice u = P.encontrar(a.orig);
      vertice v = P.encontrar(a.dest);
      if (u != v && islas.encontrar(u) != islas.encontrar(v) ) { // Los extremos de a pertenecen a componentes distintas
         P.unir(u, v);
         
         IslasConLinea.push_back(u);
         IslasConLinea.push_back(v);
         i++;
      }
   }
   return g;
