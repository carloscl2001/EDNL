...
template <typename T> class Abb {
    ...
    private:
        ...
        void inorden(std::vector<T> v, const Abb);
        void eliminarSubarbolRec(arbol* r);
        T borrarMin();
};
template <typename T>
Abb<T>& buscarElem(const T& e){
...
void Abb<T>::eliminarSubarbol(const T& e){
    Abb* subA = buscarElem(e);

    if(subA != nullptr){
        eliminarSubarbolRec(subA);
    }
}

template <typename T>
void Abb<T>::eliminarSubarbolRec(arbol* r){
    if( r != nullptr){
        if(r->izqdo != nullptr){
            eliminnarSubarbolRec(r->izq);
        }
        if(r->drcho != nullptr){
